...
class DDPM:
    ...
    def __init__(self, B, device):
        ...
        self.a_bar = torch.cumprod(self.a, dim=0)  # Cumulative product of alphas
        self.sqrt_a_bar = torch.sqrt(self.a_bar)  # Mean Coefficient for forward process
        self.sqrt_one_minus_a_bar = torch.sqrt(1 - self.a_bar)  # St. Dev. Coefficient for noise

        # Reverse diffusion variables
        # These precomputed values optimize the reverse diffusion process
        self.sqrt_a_inv = torch.sqrt(1 / self.a)  # Inverse sqrt alpha for denoising
        self.pred_noise_coeff = (1 - self.a) / torch.sqrt(1 - self.a_bar)  # Coefficient for predicted noise
    ...
    def q(self, x_0, t):
        ...
        sqrt_a_bar_t = self.sqrt_a_bar[t, None, None, None]  # Broadcast to image dimensions
    ...
    def get_context_mask(self, c, c_drop_prob):
        ...
        """
        c_mask = torch.bernoulli(torch.ones_like(c).float() - c_drop_prob).to(self.device)
        return c_mask
    ...
    def reverse_q(self, x_t, t, e_t):
        ...
        """
        t = t.int()
        pred_noise_coeff_t = self.pred_noise_coeff[t]  # Coefficient for noise removal
        ...
        if t[0] == 0:  # All t values should be the same
            ...
            return u_t + torch.sqrt(B_t) * new_noise  # Add stochastic component
...
def visualize_diffusion_process(
    ddpm, model, clip_model, clip_preprocess, embed_size, device, img_ch, img_size, timesteps_to_show=None, prompt="A cute cat", w=0.5, top_k=5
):
    """
    ...
    for idx, t_val in enumerate(timesteps_to_show):
        ...
        try:
            ...
            top_image_values, top_image_indices = image_similarities.topk(top_k)
    ...
    plt.suptitle(f"DDPM process + CLIP conditioning:'{prompt}' with weight {w}", fontsize=16)
